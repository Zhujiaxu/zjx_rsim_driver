...
bool FindTrajectoryPointAtTime(const std::vector<PlanningTrajectoryPoint>& trajectory,
                               double time,
                               PlanningTrajectoryPoint* point)
{
    if (point == nullptr || trajectory.empty())
        return false;

    if (trajectory.size() == 1)
    {
        if (std::fabs(time - trajectory.front().time) <= kTimeEpsilon)
        {
    ...
    if (time < trajectory.front().time - kTimeEpsilon ||
        time > trajectory.back().time + kTimeEpsilon)
    {
        return false;
    }
                                           ...
                                           double planningPeriod)
{
    ...
    else
    {
        ...
        point.x = vehicle.x +
                  (std::sin(nextHeading) - std::sin(vehicle.heading)) / vehicle.curvature;
        point.y = vehicle.y +
                  (-std::cos(nextHeading) + std::cos(vehicle.heading)) / vehicle.curvature;
