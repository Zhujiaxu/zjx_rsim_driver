    ...
    QpPathResult* result) const
{
    ...
    // --- corner constraints (4(n-1) ineq, i >= 1) ---
    // Front: l_i + halfLen·l'_i ∈ [Right_f + halfWid, Left_f - halfWid]
    // Rear:  l_i - halfLen·l'_i ∈ [Right_r + halfWid, Left_r - halfWid]
    for (int i = 1; i < n; ++i)
    {
        // Front upper: l_i + halfLen·l'_i ≤ Left_f - halfWid
        AddConstraintRow(&constraintTriplets, &lowerBound, &upperBound, row++,
                         {{LIndex(i), 1.0},
                          {LPrimeIndex(i), halfLen}},
                         rightFront[i] + halfWid, leftFront[i] - halfWid);

        // Front lower: l_i + halfLen·l'_i ≥ Right_f + halfWid
        AddConstraintRow(&constraintTriplets, &lowerBound, &upperBound, row++,
                         {{LIndex(i), 1.0},
                          {LPrimeIndex(i), halfLen}},
                         rightFront[i] + halfWid, leftFront[i] - halfWid);

        // Rear upper: l_i - halfLen·l'_i ≤ Left_r - halfWid
        AddConstraintRow(&constraintTriplets, &lowerBound, &upperBound, row++,
                         {{LIndex(i), 1.0},
                          {LPrimeIndex(i), -halfLen}},
                         rightRear[i] + halfWid, leftRear[i] - halfWid);

        // Rear lower: l_i - halfLen·l'_i ≥ Right_r + halfWid
        AddConstraintRow(&constraintTriplets, &lowerBound, &upperBound, row++,
                         {{LIndex(i), 1.0},
                          {LPrimeIndex(i), -halfLen}},
                         rightRear[i] + halfWid, leftRear[i] - halfWid);
    ...
    if (wJerk > 0.0)
    {
        for (int i = 0; i + 1 < n; ++i)
        {
            const double diff = output[i + 1].l_double_prime - output[i].l_double_prime;
