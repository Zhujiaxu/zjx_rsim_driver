...
bool ValidConfig(const SpeedQpOptimizerConfig &config)
{
                ...
                int expected_num_points)
{
    ...
    {
        const StPoint &lb = drivable_area.lower_boundary[i];
        const StPoint &ub = drivable_area.upper_boundary[i];
        if (!IsFinite(lb.s) || !IsFinite(lb.t) ||
            !IsFinite(ub.s) || !IsFinite(ub.t))
            return false;
        if (lb.s > ub.s)
            return false;
    }

    if (!IsFinite(reference_line_total_length) || reference_line_total_length <= kEpsilon)
        return false;
...
SpeedQpOptimizer::SpeedQpOptimizer(const SpeedQpOptimizerConfig &config)
    : config_(config)
{
}

const SpeedQpOptimizerConfig &SpeedQpOptimizer::config() const
{
    return config_;
}

void SpeedQpOptimizer::SetConfig(const SpeedQpOptimizerConfig &config)
{
    config_ = config;
}
                                ...
                                SpeedQpResult *result) const
{
    ...
    SpeedQpResult output;

    if (!ValidConfig(config_) ||
        !ValidInput(start, drivable_area, reference_line_total_length, config_.num_points))
    {
        *result = output;
        return false;
    }
    ...
    {
        AddTargetCost(&hessianTriplets, &gradient,
                      SIndex(n - 1), reference_line_total_length, wProgress);
    }
    ...
    for (int i = 1; i < n; ++i)
    {
        const double raw_lb = drivable_area.lower_boundary[static_cast<std::size_t>(i)].s;
        const double raw_ub = drivable_area.upper_boundary[static_cast<std::size_t>(i)].s;
        const double lb = (raw_lb > kEpsilon) ? raw_lb + totalMargin : raw_lb;
        const double ub = (raw_ub < reference_line_total_length - kEpsilon) ? raw_ub - totalMargin : raw_ub;
        AddConstraintRow(&constraintTriplets, &lowerBound, &upperBound, row++,
                         {{SIndex(i), 1.0}},
                         lb, ub);
    }
    ...
    // ===== extract solution =====
    output.stpoints.reserve(static_cast<std::size_t>(n));
    for (int i = 0; i < n; ++i)
    {
    ...
    if (wProgress > 0.0)
    {
        const double ds_end = solution(SIndex(n - 1)) - reference_line_total_length;
