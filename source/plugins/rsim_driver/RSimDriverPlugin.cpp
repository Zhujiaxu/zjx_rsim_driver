 ...
 *     ├── RoadPosition: roadId, s, t, Orientation(h/p/r)
...
private:
    ...
    // 解析 XOSC WorldPosition 顶点: 直接读取 x/y/h, 无需道路坐标转换
    ...
    void WriteGlobalPathCsv() const
    {
        ...
        std::fprintf(fp, "index,arc_s,delta_s,x,y,heading_rad\n");
        double arcS = 0.0;
        for (size_t i = 0; i < global_path_world_points_.size(); ++i)
        {
            ...
            std::fprintf(fp, "%zu,%.9f,%.9f,%.9f,%.9f,%.9f\n",
                         i, arcS, ds, pt.x, pt.y, pt.hdg);
        }
        std::fclose(fp);
